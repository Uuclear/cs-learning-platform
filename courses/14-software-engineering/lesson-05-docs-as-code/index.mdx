import { Quiz } from '@site/src/components/course'
...
### 练习1：创建简单的 Markdown 文档生成器（⭐）

运行 `code/examples/example-01-markdown-generator.py`，观察从 docstring 生成的 Markdown 结构。在此基础上扩展：支持**类**的文档提取，并将结果写入 `api-docs.md` 文件。

**参考答案**：`code/solutions/solution-01.py`

### 练习2：实现 Mermaid 图表生成（⭐⭐）

基于 `example-02-diagrams-as-code.py`，为给定的类定义生成 Mermaid **类图**，并为 3 步以上的交互流程生成**序列图**。

**参考答案**：`code/solutions/solution-02.py`

### 练习3：完整的 OpenAPI 生成器（⭐⭐⭐）

扩展 `example-03-api-docs.py`，为每个端点补充 `parameters` 与 `responses` 的 schema 描述，并支持导出为 JSON 或 YAML。

**参考答案**：`code/solutions/solution-03.py`

```bash
python code/examples/example-01-markdown-generator.py
python code/examples/example-02-diagrams-as-code.py
python code/examples/example-03-api-docs.py
```

### 编程挑战

完成 `exercises/challenges/` 中的两道综合挑战：

1. **挑战1**：为现有项目集成文档即代码（`challenge-01.md`）
2. **挑战2**：多源统一文档生成器（`challenge-02.md`）
