...
class HPASimulator:
    ...
    def print_summary(self, pattern: str, cpu_data: List[float], replica_data: List[int]):
        """文本方式汇总模拟结果（无需 matplotlib）"""
        avg_cpu = sum(cpu_data) / len(cpu_data)
        print(f"\n📊 {pattern} 模式摘要:")
        print(f"   平均 CPU: {avg_cpu:.1f}%")
        print(f"   副本范围: {min(replica_data)} ~ {max(replica_data)}")
        print(f"   最终副本: {replica_data[-1]}")
