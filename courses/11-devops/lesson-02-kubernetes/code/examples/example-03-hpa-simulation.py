...
模拟 Kubernetes HPA 如何根据 CPU 负载自动调整 Pod 数量（纯文本输出，无额外依赖）。
"""
    ...
    def simulate_load_pattern(self, load_pattern: str, duration: int = 30) -> Tuple[List[float], List[int]]:
        ...
        for t in range(duration):
            if load_pattern == "steady":
                ...
                cpu_usage = (80 + random.gauss(0, 10)) if t % 10 == 0 else (30 + random.gauss(0, 5))
...
def main():
    ...
    for pattern in patterns:
        ...
        print(f"  摘要: 平均 CPU={avg_cpu:.1f}%, 峰值副本={max(replica_data)}, 谷值副本={min(replica_data)}")

    print("\n✅ HPA 模拟完成！")
    print("💡 观察要点:")
    print("   - 副本数随 CPU 与目标值的比值伸缩")
    print("   - min/max 副本数会钳制伸缩范围")
    print("   - 真实集群中 HPA 有同步周期（通常 15–30 秒）")
