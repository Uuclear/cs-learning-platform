...
数据治理是一套组织流程、角色、策略和标准，用于确保数据在整个组织中的可用性、完整性、一致性、可审计性和安全性。它不仅仅是技术问题，更是业务和管理问题。
...
在 `example-01-data-quality.py` 的基础上，为 `check_data_accuracy` 增加 **电话号码** 验证规则（`rule == "phone"`）：值必须是字符串，且去掉 `-` 和空格后全部为数字。

**提示**：可参考 `code/solutions/solution-01.py` 中的 `phone` 规则实现。

### 练习2：血缘影响分析（⭐⭐）

使用 `DataLineageTracker`，注册以下处理链：`raw_orders` → `cleaned_orders` → `monthly_sales_report`。编写代码打印 `monthly_sales_report` 的**全部上游节点**，以及从 `raw_orders` 到 `monthly_sales_report` 的**完整路径**。

**提示**：使用 `get_upstream()` 和 `find_impact_path()`，答案思路见 `code/solutions/solution-02.py`。

### 练习3：数据目录搜索（⭐⭐）

向 `SimpleDataCatalog` 注册至少 3 个数据资产，要求其中一个带 `pii` 标签。实现：按关键词 `"order"` 搜索，再按标签 `["pii"]` 过滤，并打印匹配资产的 `asset_id` 与 `owner`。

**提示**：参考 `code/solutions/solution-03.py` 中的 `search_assets` 用法。
